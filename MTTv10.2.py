...
def abletime():
    return np.arange(wpn)[masktime(cN)]


def masktime(n):
    l1 = PIL(ICL, n)[0]
    return (np.sum(GP[np.flatnonzero(GC[:, n])], axis=0) == 0) * (LP[l1] < MLP[l1])


def Even(arr):
    timeset = np.intersect1d(EP[LiP[cN, EP] * LiP[cN, VEP] == 1], abletime())
    eap = np.array([], int)
...
def changing(arr, xind, yind, num):
    for i in xind:
        for j in yind:
            arr[i, j] = num
...
def findEG4(n):
    for j in np.flatnonzero(GC[:, n]):
        DCiP = np.zeros(wpn,int)
        np.putmask(DCiP, (GC.T[n] @ GP - GP[j] == 0)*masktime(n), 1)
        if np.min(np.sum(DCiP[EP] * DCiP[VEP] == 1)) >= 2 - kind % 2:
            return j
    return -1
...
while dead == 0:
    ...
    if out == 1:
        ...
        while out == 0:
            ...
            if cN in ICL:
                if old == 0:
                    LiP = np.append(LiP, np.zeros((LCN[lN], wpn), int), axis=0)
                    CiP = np.append(CiP, np.zeros((LCN[lN], wpn), int), axis=0)
                if lN == l - 1:
                    np.putmask(LiP[cN:], GC.T[cN:] @ GP == 0, 1)
                else:
                    np.putmask(LiP[cN:], GC.T[cN:cN + LCN[lN]] @ GP == 0, 1)
                for i in range(cN,len(LiP),1):
                    CiP[i] = LiP[i]*masktime(i)
                EC1 = np.array([], int)
                EC2 = np.array([], int)
                if kind == 0 or kind == 1 or kind == 3:
                    sums = np.sum(LiP[cN:, EP] * LiP[cN:, VEP] == 1, axis=1)
                    if np.min(sums) < 2 - kind % 2:
                        EC1 = cN + np.argwhere(sums == np.min(sums)).reshape(-1)
                sums2 = np.sum(LiP[cN:] == 1, axis=1)
                if np.min(sums2) < (12 - 2 * kind) // 3:
                    EC2 = (cN + np.argwhere(sums2 == np.min(sums2)).reshape(-1))
                EC = np.union1d(EC1, EC2)
                if len(EC) > 0:
                    go = 1
                    back = 1
                for i in EC:
                    if i in EC1 and i in EC2:
                        if findEG3(i) > -1:
                            RE = np.unique(np.append(RE, i * s + findEG3(i)))
                        elif findEG(i) > -1:
                ...
                EC4 = np.array([], int)
                if kind == 0 or kind == 1 or kind == 3:
                    sums3 = np.sum(CiP[ICL[lN]:, EP] * CiP[ICL[lN]:, VEP] == 1, axis=1)
                    if np.min(sums3) < 2 - kind % 2:
                        EC3 = ICL[lN] + np.argwhere(sums3 == np.min(sums3)).reshape(-1)
                sums4 = np.sum(CiP[ICL[lN]:] == 1, axis=1)
                if np.min(sums4) < (12 - 2 * kind) // 3:
                    EC4 = (ICL[lN] + np.argwhere(sums4 == np.min(sums4)).reshape(-1))
                EC0 = np.union1d(EC3, EC4)
                if len(EC0) > 0:
                    go = 1
                    back = 1
                for i in EC0:
                    if i in EC3 and i in EC4:
                        if findEG6(i) > -1:
                            RE = np.unique(np.append(RE, i * s + findEG6(i)))
                        elif findEG4(i) > -1:
                            RE = np.unique(np.append(RE, i * s + findEG4(i)))
                        elif findEG5(i) > -1:
                            RE = np.unique(np.append(RE, i * s + findEG5(i)))
                    elif i in EC3:
                        if findEG4(i) > -1:
                            RE = np.unique(np.append(RE, i * s + findEG4(i)))
                    else:
            ...
            while go == 0:
                if RpL[-1] == 0 or pN == RpL[-1]:
                    ...
                    if gtest == -1:
                        ...
                        CiP[cN] = np.zeros(wpn, int)
                        plusing(CiP, [cN], SelP, 1)
                        str1 = ''
                        for i in range(wpn):
                            if i not in SelP:
                                str1 = str1 + ' ' * len(str(i)) + ' '
            ...
            while back == 1:
                ...
                if cN == -1:
                    ...
                    if pN == RpL[-1]:
                        ...
                        CiP = CiP[:cN]
                        for i in range(cN,len(LiP),1):
                            CiP = np.append(CiP,[LiP[i]*masktime(i)],axis = 0)
                        if lN > PIL(ICL, cN)[0]:
                            LiP = LiP[:ICL[PIL(ICL, cN)[0] + 1]]
