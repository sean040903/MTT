...
def Even(arr):
    timeset = EP[(AiP[cN]*masktime(cN))[EP] * (AiP[cN]*masktime(cN))[VEP] == 1]
    eap = np.array([], int)
...
def Odd(arr):
    timeset = np.arange(wpn)[(AiP[cN] == 1)*masktime(cN)]
    oap = np.array([], int)
    for i in range(weekly):
...
def AfindEC(n):
    for j in np.flatnonzero(GC[:, n]):
        ...
        np.putmask(DCiP, AiP[n] - GP[j] * masktime(n) == 0, 1)
        if (np.min(np.sum(DCiP == 1)) >= (12 - 2 * kind) // 3) and (np.min(np.sum(DCiP[EP] * DCiP[VEP] == 1)) >= 2 - kind % 2):
            return j
    return -1
...
def printmtt(n1, n2):
    ...
    for i in range(cN):
        if i == cN - 1:
            printstr = printstr + '\033[33;1m' + str(i) + ' ' * (3 - len(str(i))) + ':V' + CP[i] + ' ' + '\033[0m'
        else:
    ...
    printone = printstr + 'Found:' + str(n1) + '       checked SC: ' + str(n2) + '         maxcN: ' + str(maxcN - 1) + '        RE: ' + str(len(RE))
...
while dead == 0:
    ...
    if out == 1:
        ...
        while out == 0:
            ...
            kind = KL[lN]
            go = 0
            for checkc in range(cN+1,c,1):
                if cN == 0 or np.sum(RAiP[-1,checkc]) != np.sum(AiP[checkc]):
                    checkk = KC[checkc]
                    checksum2 = np.sum(AiP[checkc])
                    if checksum2 < 4- (2*checkk)//3:
                        RE = np.unique(np.append(RE,s*np.flatnonzero(GC[:,cN]*GC[:,checkc])+checkc))
                        go = 1
                        back = 1
                    else:
                        if checkk == 0 or checkk == 1 or checkk == 3:
                            checksum1 = np.sum(AiP[checkc,EP] * AiP[checkc,VEP])
                            if checksum1 < 2 - checkk % 2:
                                RE = np.unique(np.append(RE,s*np.flatnonzero(GC[:,cN]*GC[:,checkc])+checkc))
                                go = 1
                                back = 1
            AP = APTType(TType)
            ...
            while go == 0:
                if RpL[-1] == 0 or pN == RpL[-1]:
                    ...
                    if gtest == -1:
                        ...
                        for i in SelP:
                            AiP[cN,i] = 1
                        for i in range(cN + 1, c, 1):
                            if PIL(ICL, i) == PIL(ICL, cN) or i in htcc[htcc > cN]:
