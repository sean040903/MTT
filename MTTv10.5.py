...
# 추가 함수
def SelT(kc1, apptype1, pn1):
    if kc1 == 4:
        return np.array([apptype1[pn1]])
    elif kc1 == 3:
        return np.array([apptype1[pn1], apptype1[pn1] + 1])
    elif kc1 == 2:
        return np.array([apptype1[pn1][0], apptype1[pn1][1]])
    elif kc1 == 1:
        return np.array([apptype1[pn1][0], apptype1[pn1][0] + 1, apptype1[pn1][1]])
    else:
        return np.array([apptype1[pn1][0], apptype1[pn1][0] + 1, apptype1[pn1][1], apptype1[pn1][1] + 1])


def abletime(cn1):
    return np.arange(wpn)[masktime(cn1)]


def masktime(cn2):
    ln1 = PIL(ICL, cn2)[0]
    return (np.sum(GP[np.flatnonzero(GC[:, cn2])], axis=0) == 0) * (LP[ln1] < MLP[ln1])


def Even(ttype1, cn3):
    timeset1 = EP[((AiP[cn3] * masktime(cn3))[EP]) * (AiP[cn3] * masktime(cn3))[VEP] == 1]
    eap = np.array([], int)
    for i in range(weekly):
        if i not in CAD or ttype1[SDPI[np.where(CAD == i)[0]]] == 1:
            eap = np.append(eap, np.intersect1d(np.arange(IPD[i], ttype1[i] + 2, 2), timeset1))
    return np.sort(eap)


def Odd(ttype2, cn4):
    timeset2 = np.arange(wpn)[(AiP[cn4] == 1) * masktime(cn4)]
    oap = np.array([], int)
    for i in range(weekly):
        n1 = ttype2[SDPI[np.where(CAD == i)[0]]]
        n2 = NEP[np.where(NEPI == i)[0]]
        if i in CAD:
            if n1 == 1:
                oap = np.append(oap, np.intersect1d(np.arange(IPD[i], ttype2[i] + 2, 2), timeset2))
        else:
            oap = np.append(oap, np.intersect1d(np.arange(IPD[i], ttype2[i] + 2, 2), timeset2))
        if i not in CAD or n1 == 1 and DPN[i] % 2 == 1:
            if len(n2) > 0 and n2 in timeset2:
                oap = np.append(oap, n2)
    for j in NZBF(ttype2[-1]):
        if VEP[j] in timeset2:
            oap = np.append(oap, VEP[j])
    return np.sort(oap)


def APTType(ttype3, cn5):
    kc2 = KC[cn5]
    if kc2 == 3:
        ap = Even(ttype3, cn5)
    elif kc2 == 4:
        ...
        if len(AP1) > 0:
            for p1 in AP1:
                AP2 = Odd(CTType(4, p1, ttype3), cn5)
        ...
        if len(AP1) > 0:
            for p1 in AP1:
                AP2 = Odd(CTType(3, p1, ttype3), cn5)
        ...
        if len(AP1) > 0:
            for p1 in AP1:
                AP2 = Even(CTType(3, p1, ttype3), cn5)
                AP2 = AP2[AP2 > FPD[date(p1)[0]]]
                if len(AP2) > 0:
                    for p2 in AP2:
                        ap = np.append(ap, [[p1, p2]], axis=0)
...
def LPTType(ttype3, cn6):
    ...
    if kc3 == 3:
        ...
        if len(AP1) > 0:
            for p1 in AP1:
                AP2 = Odd(CTType(4, p1, ttype3), cn6)
                AP2 = AP2[AP2 > FPD[date(p1)[0]]]
                lp += len(AP2)
    elif kc3 == 1:
        AP1 = Even(ttype3, cn6)
        if len(AP1) > 0:
            for p1 in AP1:
                AP2 = Odd(CTType(3, p1, ttype3), cn6)
                AP2 = AP2[(AP2 > FPD[date(p1)[0]]) + (AP2 < IPD[date(p1)[0]])]
                lp += len(AP2)
    else:
        AP1 = Even(ttype3, cn6)
        if len(AP1) > 0:
            for p1 in AP1:
                AP2 = Even(CTType(3, p1, ttype3), cn6)
                AP2 = AP2[AP2 > FPD[date(p1)[0]]]
                lp += len(AP2)
    return lp
...
def GTest(selp1, cn7):
    timeset = abletime(cn7)
    for i in selp1:
        if i not in timeset:
            return np.argmax(GC[:, cn7] > 0) + s * cn7
    return -1
        ...
        while out == 0:
            if old == 0:
                if np.min(lAPcN[NcN == 1]) == 0:
                    ...
                    back = 1
                    go = 1
                elif np.min(lAPcN[NcN == 1]) == 1:
                    cN = np.argwhere(lAPcN[NcN == 1] == 1)[0]
                    maxcN = max(maxcN, c - np.sum(NcN))
                    pN = 0
                else:
                    cN = np.argmax(NcN)
                    maxcN = max(maxcN, c - np.sum(NcN))
            ...
            if np.sum(NcN) == 0:
                ...
                for c1 in RcN:
                    TType = RTType[c1]
                    ki = KC[c1]
                    Selt2 = SelT(ki, APTType(TType,cN), RpN[c1])
                    l1 = PIL(ICL, c1)
            ...
            if back == 0:
                ...
                AiP[cN] = np.zeros((wpn,), dtype=int)
                for i in SelP:
                    AiP[cN, i] = 1
                for i in np.flatnonzero(NcN):
                    if PIL(ICL, i) == PIL(ICL, cN) or i in htcc:
                        AiP[i] = AiP[i] * masktime(i)
                        lAPcN[i] = LPTType(TType, i)
