...
def APTType(AiP3, cn9, ttype6, num=0):
    ...
    if kc4 == 4:
        ...
        apve = VEP[np.intersect1d(np.flatnonzero(AiP3[cn9, EP]), NZBF(ttype6[-1]))]
        return np.append(ape, apve)
    elif kc4 == 3:
...
def LPTType(AiP3, cn9, ttype6, num=0):
    ...
    if num > 0:
        kc4 = 5 - num
    if kc4 == 4:
        return len(AiP3[cn9, EP] == 1) + len(VEP[np.intersect1d(np.flatnonzero(AiP3[cn9, EP]), NZBF(ttype6[-1]))])
    elif kc4 == 3:
        return len(AiP3[cn9, EP] == 1)
    elif kc4 == 2:
        lp2 = 0
        AP1 = APTType(AiP3, cn9, ttype6, num=1)
        for i in AP1:
        ...
        for i in AP1:
            AiP4 = CAiP(AiP3, cn9, 3, ttype6, [i,i+1])
            lp3 += LPTType(AiP4, cn9, CTType(3, i, ttype6), num=1)
        return lp3
    else:
        lp4 = 0
        AP1 = APTType(AiP3, cn9, ttype6, num=2)
        ...
        while out == 0:
            if old == 0:
                if np.min(lAPcN[NcN == 1]) == 0:
                    ...
                    cN = np.intersect1d(np.argwhere(lAPcN == 1).reshape(-1),np.flatnonzero(NcN))[0]
