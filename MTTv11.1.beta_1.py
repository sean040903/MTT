...
def CAiP(AiP1, cn7, kc4, ttype4, selp1):  # TType을 변경하기 전에 진행을 해야 한다.
    ...
    for d1 in np.unique(PIL(IPD, np.array([selp1]).reshape(-1))):
...
def APTType(AiP3, cn9, ttype6, num=0):
    ...
    if kc4 == 4:
        ...
        for i in AP1:
            AiP4 = CAiP(AiP3, cn9, 4, ttype6, np.array([i]))
            AP2 = APTType(AiP4, cn9, CTType(4, np.array([i]), ttype6), num=1)
            AP2 = AP2[AP2 > i]
            for j in AP2:
        ...
        for i in AP1:
            AiP4 = CAiP(AiP3, cn9, 3, ttype6, np.array([i, i + 1]))
            AP2 = APTType(AiP4, cn9, CTType(3, np.array([i]), ttype6), num=1)
            for j in AP2:
        ...
        for i in AP1:
            AiP4 = CAiP(AiP3, cn9, 3, ttype6, np.array([i, i + 1]))
            AP2 = APTType(AiP4, cn9, CTType(3, np.array([i]), ttype6), num=2)
            AP2 = AP2[AP2 > i]
            for j in AP2:
...
def LPTType(AiP3, cn9, ttype6, num=0):
    ...
    if kc4 == 4:
        ...
        AP1 = APTType(AiP3, cn9, ttype6, num=1)
        for i in AP1:
            AP2 = APTType(CAiP(AiP3, cn9, 4, ttype6, np.array([i])), cn9, CTType(4, np.array([i]), ttype6), num=1)
            AP2 = AP2[AP2 > i]
            lp2 += len(AP2)
        return lp2
    elif kc4 == 1:
        ...
        AP1 = APTType(AiP3, cn9, ttype6, num=2)
        for i in AP1:
            lp3 += LPTType(CAiP(AiP3, cn9, 3, ttype6, np.array([i, i + 1])), cn9, CTType(3, np.array([i]), ttype6), num=1)
        return lp3
    else:
        lp4 = 0
        AP1 = APTType(AiP3, cn9, ttype6, num=2)
        for i in AP1:
            AP2 = APTType(CAiP(AiP3, cn9, 3, ttype6, np.array([i, i + 1])), cn9, CTType(3, np.array([i]), ttype6), num=2)
            AP2 = AP2[AP2 > i]
            lp4 += len(AP2)
        return lp4
...
def PIL(arr1, ip):
    ...
    for i in ip:
        pil1 = np.append(pil1,np.sum(arr1 <= ip[i])-1)
    return pil1
...
def CTType(kc4, selp1, ttype4):
    arr1 = np.copy(ttype4)
    for i in range(weekly):
        if ttype4[i] < DMEP[i]:
            for j in selp1:
                if j == EP[PIL(EP, np.array([ttype4[i]]))[0]]:
                    arr1[i] += 2
    dates = np.unique(PIL(IPD, np.array([selp1]).reshape(-1)))
    for i in dates:
        if i in DCD:
            arr1[weekly + np.where(DCD == i)[0]] = 1
    if kc4 == 1:
        if selp1[1] in EP or selp1[1] in VEP:
            arr1[-1] = BFN(np.union1d(NZBF(ttype4[-1]), PIL(EP, np.array([selp1[1]]))))
    elif kc4 == 2 or kc4 == 4:
        for i in selp1:
...
def sortition(arr):
    colsort = np.empty(shape=(0), dtype=np.int64)
    for l1 in range(l):
        ind = np.arange(LCN[l1]) + ICL[l1]
        colsort = np.append(colsort, np.argsort(np.argmax(arr[:, ind], axis=0)) + ICL[l1])
    return arr[:, colsort]
...
datepernum = ' ' * 8
for i in range(wpn):
    datepernum = datepernum + str(i - IPD[PIL(IPD, np.array([i]))[0]] + 1) + ' ' * (len(str(i)) - 1) + ' '
        ...
        while out == 0:
            if old == 0:
                if np.min(lAPcN[NcN == 1]) == 0:
                    ...
                    cN = AcN[np.argmax(np.sum(GC[YgN == 1][:, AcN], axis=0) * g + np.sum(GC[:, AcN], axis=0))]
            ...
            if np.sum(NcN) == 0:
                ...
                for s1 in range(s):
                    sctd = ScT[s1]
                    for t1 in DP:
                        l1 = PIL(ICL, np.array([sctd[t1 - 1]]))[0]
                        l2 = PIL(ICL, np.array([sctd[t1]]))[0]
                        tdw += DI[l1][l2]
            ...
            if back == 0:
                RYgN = np.append(RYgN, [YgN], axis=0)
                if np.min(lAPcN[NcN == 1]) > 1:
                    YgN[GC[:, cN] == 0] = 0
                if np.sum(YgN) == 0:
                    YgN = np.ones(g, int)
                ...
                for i in np.flatnonzero(NcN):
                    if PIL(ICL, np.array([i]))[0] == PIL(ICL, np.array([cN]))[0] or i in htcc:
