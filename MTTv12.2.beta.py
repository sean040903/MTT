...
def LPTType(AiP3, ttype6, LP3, cn9, num=0):
    ...
    if num > 0:
        kc4 = 5 - num
    if kc4 == 4:
        return np.sum(AiP3[cn9, AEP] == 1) + len(np.intersect1d(np.flatnonzero(AiP3[cn9, EP] == 1), NZBF(ttype6[-1])))
    elif kc4 == 3:
        ...
        for ap1 in AP1:
            lp3 += LPTType(CAiP(AiP3, ttype6, ap1, LP3, cn9), CTType(ap1, ttype6), LP3, cn9, num=1)
        return lp3
    else:
        lp4 = 0
        AP1 = APTType(AiP3, ttype6, LP3, cn9, num=2)
