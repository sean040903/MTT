...
def CAiP(AiP1, ttype4, selp1, LP1, cn7):  # TType을 변경하기 전에 진행을 해야 한다.
    ...
    for i in Ck:
        AiP2[NcN, i:i + 2] *= -1
    for d1 in ray.get(date.remote(selp1)):
...
def APTType(AiP3, ttype6, LP2, cn9, num=0):
    ...
    if kc4 == 4:
        ...
        for ap1 in AP1:
            ...
            AP2 = AP2[np.flatnonzero(ray.get(date.remote(AP2[:, 0])) > ray.get(date.remote(ap1)))]
            for ap0 in AP2:
        ...
        for s0 in range(len(arr)):
            for i in range(weekly):
                for j in range(daily):
                    c5 = list3[s0][i][j]
                    if c5 < c:
                        l1 = ray.get(PIL.remote(ICL, np.array([c5])))[0]
                        pc = (c5 - ICL[l1] + 1)
