...
# 변경 함수
@profile
def CAiP_1(AiP1, ttype4, selp1):  # TType을 변경하기 전에 진행을 해야 한다.
    AiP2 = np.copy(AiP1)
...
@profile
def CAiP_2(AiP1, ttype4, selp1, LP1, cn7, Ncn7):  # TType을 변경하기 전에 진행을 해야 한다.
    AiP2 = np.copy(AiP1)
...
def CAiP(AiP1, ttype4, selp1, LP1, cn7, Ncn7):
    return CAiP_2(CAiP_1(AiP1, ttype4, selp1), ttype4, selp1, LP1, cn7, Ncn7)
...
def APTType(AiP3, ttype6, LP2, cn9, num=0):
    ...
    if kc4 == 4:
        ...
        for ap1 in AP1:
            AiP4 = CAiP(AiP3, ttype6, LP2, ap1, cn9, np.array([cn9]))
        ...
        for ap1 in AP1:
            AiP4 = CAiP(AiP3, ttype6, ap1, LP2, cn9, np.array([cn9]))
            AP2 = APTType(AiP4, CTType(ap1, ttype6), LP2, cn9, num=1)
            AP2 = AP2[PIL(IPD, AP2[:, 0]) != date(ap1)]
            for ap0 in AP2:
...
def LPTType(AiP3, ttype6, LP3, cn9, num=0):
    ...
    if num > 0:
        kc4 = 5 - num
    if kc4 == 4:
        return len(np.flatnonzero(AiP3[cn9, AEP] == 1)) + len(np.intersect1d(np.flatnonzero(AiP3[cn9, VEP] == 1), NZBF(ttype6[-1])))
    elif kc4 == 3:
        return len(np.flatnonzero(AiP3[cn9, EP] == 1))
    elif kc4 == 2:
        lp2 = 0
        AP1 = APTType(AiP3, ttype6, LP3, cn9, num=1)
        for ap1 in AP1:
            AP2 = APTType(CAiP(AiP3, ttype6, ap1, LP3, cn9, np.array([cn9])), CTType(ap1, ttype6), LP3, cn9, num=1)
            AP2 = AP2[PIL(IPD, AP2[:, 0]) > date(ap1)]
            lp2 += len(AP2)
        return lp2
    elif kc4 == 1:
        ...
        AP1 = APTType(AiP3, ttype6, LP3, cn9, num=2)
        for ap1 in AP1:
            AP2 = APTType(CAiP(AiP3, ttype6, ap1, LP3, cn9, np.array([cn9])), CTType(ap1, ttype6), LP3, cn9, num=2)
            AP2 = AP2[PIL(IPD, AP2[:, 0]) > date(ap1)]
            lp4 += len(AP2)
        return lp4
...
def CTType(selp1, ttype4):
    return CTType_2(selp1,CTType_1(selp1,ttype4))
        ...
        while out == 0:
            ...
            if len(SuLlcN) > 0:
                ...
                for i in tqdm(range(sum(Renum)), desc=str(len(SuRcN.T)), ascii=True):
                    ...
                    GP = SuGP[i]
                    SuAiP[i] = np.copy(CAiP(SuAiP[i], SuTType[i], AP, SuLP[i], SucN[i], SuNcN[i]))
