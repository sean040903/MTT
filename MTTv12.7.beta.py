...
def APTType(AiP3, ttype6, LP2, cn9, num=0):
    ...
    if kc4 == 4:
        ...
        for ap1 in AP1:
            ...
            AP2 = APTType(CAiP(AiP3, ttype6, ttype7, ap1, LP2, cn9, np.array([cn9])), ttype7, LP2, cn9, num=1 + (kc4 < 1))
            if kc4 == 1:
...
def LPTType(AiP3, ttype6, LP3, cn9, num=0):
    ...
    if num > 0:
        kc4 = 5 - num
    if kc4 == 4:
        return np.sum(AiP3[cn9, AEP] == 1) + len(np.intersect1d(np.flatnonzero(AiP3[cn9, VEP] == 1), NZBF(ttype6[-1]), assume_unique=True))
    elif kc4 == 3:
