...
def APTType(AiP3, ttype6, LP2, cn9, num=0):
    ...
    if num > 0:
        kc4 = 5 - num
    if kc4 == 4:
        apve = VEP[np.intersect1d(np.flatnonzero(AiP3[cn9, VEP] == 1), NZBF(ttype6[-1]), assume_unique=True)]
        ...
        aps = np.empty((0, 1 + (kc4 < 1)), int)
        AP1 = APTType(AiP3, ttype6, LP2, cn9, num=1 + (kc4 < 2))
        SAP = np.array([], int)
        for ap1 in AP1:
            ttype7 = CTType(ap1, ttype6)
            LP2[PIL(ICL, np.array([cn9]))[0], ap1] += 1
            AP2 = APTType(CAiP(AiP3, ttype6, ttype7, ap1, LP2, cn9, np.array([cn9], int)), ttype7, LP2, cn9, num=1 + (kc4 < 1))
            if kc4 == 1:
                aps = np.append(aps, AP2, axis=0)
                SAP = np.append(SAP, len(AP2))
            else:
...
def LPTType(AiP3, ttype6, LP3, cn9, num=0):
    ...
    if num > 0:
        kc4 = 5 - num
    if kc4 == 4:
        return np.sum(AiP3[cn9, AEP] == 1) + np.sum(np.intersect1d(np.flatnonzero(AiP3[cn9, VEP] == 1), NZBF(ttype6[-1]), assume_unique=True))
    elif kc4 == 3:
...
while dead == 0:
    ...
    if out == 1:
        ...
        CAiP(fAiP, fTType, fTType, np.array([0], int), np.zeros((l, wpn), int), 0, np.arange(c))
