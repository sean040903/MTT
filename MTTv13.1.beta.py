...
# @profile
@njit(int64[:](int64[:],int64[:]),nogil=True, cache=True)
...
def CAiPt(AiPt1, ttype4, ttype5, selp1, LP1, cn7, Ncn7):
    ...
    if cn7 not in Ncn7:
        ...
        if KC[cn7] != 1:
            AiP2[cn7][:IPD[date(selp1)][0]] = 0
    return vectornumizewpn(AiP2)


# @profile
# @njit(int64[:, :](int64[:], int64[:], int64[:, :], int64, int64), nogil=True, cache=False)
def APTType(AiPt3, ttype6, LP2, cn9, num):
    ...
    if kc4 == 4:
        ...
        AiP3 = vectortrizewpn(AiPt3)
        ape = EP[(AiP3[cn9][EP] == 1) * (AiP3[cn9][VEP] == 1)].reshape(-1, 1)
        return np.append(ape, ape + 1, axis=1)
    else:
        aps = np.empty(shape=(0, 1 + (kc4 < 1)), dtype=np.int64)
        AP1 = APTType(AiPt3, ttype6, LP2, cn9, 1 + (kc4 < 2))
        SAP = np.array([], dtype=np.int64)
        for ap1 in AP1:
            ttype7 = CTType(ap1, ttype6)
            LP2[PIL(ICL, np.array([cn9]))[0]][ap1] += 1
            AP2 = APTType(CAiPt(AiPt3, ttype6, ttype7, ap1, LP2, cn9, np.array([cn9], dtype=np.int64)), ttype7, LP2, cn9, 1 + (kc4 < 1))
            if kc4 == 1:
...
# @profile
@njit(int64(int64[:], int64[:], int64[:, :], int64, int64), nogil=True, cache=True, parallel=True)
    ...
    if kc4 == 4:
        ...
        if kc4 == 2:
            ...
            for ap1 in prange(len(apve)):
                ttype7 = CTType(np.array([apve[ap1]]), ttype6)
                lp2 += np.sum(vectortrizewpn(CAiPt(AiPt3, ttype6, ttype7, np.array([apve[ap1]]), LP3, cn9, np.array([cn9])))[cn9] == 1)
            for ap1 in prange(len(apae)):
                ttype7 = CTType(np.array([apae[ap1]]), ttype6)
                lp2 += np.sum(vectortrizewpn(CAiPt(AiPt3, ttype6, ttype7, np.array([apae[ap1]]), LP3, cn9, np.array([cn9])))[cn9] == 1)
            return lp2
        else:
            ape = EP[AiP3[cn9][EP] == 1]
            for apei in prange(len(ape)):
                ttype7 = CTType(np.array([ape[apei], ape[apei] + 1]), ttype6)
                AiP4 = vectortrizewpn(CAiPt(AiPt3, ttype6, ttype7, np.array([ape[apei], ape[apei] + 1]), LP3, cn9, np.array([cn9])))
                if kc4 == 0:
                    lp2 += np.sum((AiP4[cn9][EP] == 1) * (AiP4[cn9][VEP] == 1))
                else:
                    lp2 += (np.sum(AiP4[cn9][AEP] == 1) + np.sum((AiP4[cn9][VEP] == 1) * (bitizettype(ttype7[-1]) == 1)))
            return lp2
...
trizewpn(0)
numizewpn(np.zeros(wpn, dtype=np.int64))
vectornumizewpn(np.zeros((l, wpn), dtype=np.int64))
vectortrizewpn(np.zeros(l, dtype=np.int64))
fAiPt = vectornumizewpn(fAiP)
...
while dead == 0:
    ...
    if out == 1:
        ...
        CAiPt(fAiPt, fTType, fTType, np.array([0], dtype=np.int64), np.zeros((l, wpn), dtype=np.int64), 0, C)
        ...
        while out == 0:
            ...
            if len(SuLlcN) > 0:
                ...
                APcN1d = np.array([], dtype=np.int64)
                IAD = np.array([], dtype=np.int64)
