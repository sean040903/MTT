...
fTType = np.append(fTType, np.zeros(len(EP)))
...
def MOPK(OPK,TType,kind):
    if kind == 0:
        ...
        OPK[VEP]*=TType[-len(EP):]
        return OPK

@njit(nogil=True,cache=True)
def COPK(OPK,date,TType,kc):
    if date in DCD and TType[weekly+np.argwhere(DCD,date)]==1:
        OPK[IPD[CAD[DCD==date]]:FPD[CAD[DCD==date]]+1]=np.copy(OPK[IPD[date]:IPD[date+1]])
    OPK[IPD[date]:IPD[date+1]]=0
    if not kc%2:
        OPK[IPD[date]]=0
    return MOPK(OPK,TType,kc>0)



# @profile
@njit(nogil=True, cache=True)
def APTType(OPKt, ttype6, cn9):
    OPK0 = dectonary0d(OPKt,3,wpn,0)
    kc=KC[cn9]
    ap1 = MOPK(OPK0,ttype6,(kc==2)+(kc==4))
    if kc>2:
        return (9-2*kc)*2**ap1
    else:
        ap = np.array([],dtype=np.int64)
        date = np.atleast_1d(DP[ap1])
        for i in range(weekly-1):
            if i in date:
                for j in ap1[date==i]:
                    ap = np.append(ap,(1+2*(kc<2))*2**j+(1+2*(kc<1))*2**COPK(OPK0,i,ttype6,kc))



# @profile
@njit(nogil=True, cache=True, parallel=True)
def LPTType(OPKt, ttype6, cn9):
    OPK0 = dectonary0d(OPKt,3,wpn,0)
    kc = KC[cn9]
    ap1 = MOPK(OPK0,ttype6,(kc==2)+(kc==4))
    if kc>2:
        ...
        lp = 0
        date = np.atleast_1d(DP[ap1])
        for i in prange(weekly-1):
            if i in date:
                lp+=np.sum(date==i)*len(COPK(OPK0,i,ttype6,kc))
        return lp
