...
@jit(nopython=True, parallel=True, cache=True, fastmath=True, forceobj=True)
def ProS(arr1, arr2):
    pros1 = 1
...
# 행렬-원소 선택 함수들#
@jit(nopython=True, cache=True, fastmath=True, forceobj=True)
def PIL(arr, n):
    if arr[-1] <= n:
        return len(arr) - 1
    else:
...
@jit(nopython=True, parallel=True, cache=True, fastmath=True, forceobj=True)
def dates(arr):
    Dates = np.zeros(len(arr))
    for i4 in range(len(arr)):
        Dates[i4] = PIL(IPD, arr[i4])
    return Dates
...
@jit(i8[:](i8), parallel=True, cache=True, fastmath=True, forceobj=True)
def NZBF(n):
    nzbf = np.array([], int)
...
while control == 0:
    ...
    if cN == 0:
        ...
        while cN < c:
            ki = PIL(ICLK, cN)
            RpL = np.append(RpL, [len(APTType(ki, RTType[-1]))])
            while pN < RpL[-1] + 1:
                if cN == c:
                    CpT = np.zeros((c, wpn))
                    SpT = np.zeros((s, wpn))
                    ...
                    if sdca == 1:
                        ...
                        CTSN = np.array([], int)
                        for c1 in range(c):
                            CTSN = np.append(CTSN, [np.sum(SC[:c1])])
                        SCDSD = np.array([], int)
                        for i in range(nds):
                            ADS3 = np.copy(ADS[i])
                            qdsd = 0
                ...
                if control == 0:
                    ...
                    lN = PIL(ICL, cN)
                    i = 0
                    len111 = np.sum(GSC[:,cN])
                    if came == 0:
                        pN = 0
                    while i < len111:
                        gN = np.nonzero(GSC[:, cN])[0][i]
                        ...
                        if i == len111:
                            if MEx(SelM(CLP, SelT(ki, APs, pN)), lN) == MCPL[lN]:
                                ...
                                GSTP = MulS(GSTP, np.nonzero(GSC[:, cN])[0], ProS(Prime, Selt1))
                                ...
                                RpL = np.append(RpL, [len(APTType(ki, RTType[-1]))])
                                print(RpN)
                                printone = '\r' + 'class:' + str(cN) + ',' + " ".join(np.array_str(RpN))
                                print(printone, end='')
    if control == 1:
        end = 1
        i = 0
        while i < len(RWC):
            c4 = RWC[-i - 1]
            if np.nonzero(GSC[:, c4])[0][0] > 1 and LCN[PIL(ICL, c4)] > 1:
