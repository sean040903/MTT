...
def CTType(ki, SelP, arr):
    ...
    for j in range(len(Selp)):
        if DATES[j] in DCD:
            arr2[weekly + np.where(DCD == DATES[j])[0][0]] = 1
        if bfs[-j - 1] == 1:
    ...
    if len(Selp) > 1:
        if Selp[0] + np.sum(DPN[DATES[0]:DATES[1]]) == Selp[1] and DATES in SDPD and bfs[-1] == bfs[-2]:
            i0 = weekly + np.where(DCD == DATES[0])[0][0]
            print(np.where(DCD == DATES[0])[0][0])
            if arr[i0] == 0:
                arr2[i0] = 0
    return arr2
                    ...
                    while i < len111:
                        ...
                        if i == len111:
                            if MEx(SelM(CLP, SelT(ki, APs, pN)), lN) == MCPL[lN]:
                                ...
                                RpL = np.append(RpL, [len(APTType(ki, RTType[-1]))])
                                list1 = RpN.tolist()
                                printone = '\r' + 'class:' + str(cN) + ',' + " ".join(map(str,list1))
                                print(printone, end='')
    if control == 1:
        end = 1
        i = 0
