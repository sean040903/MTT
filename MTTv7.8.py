...
def APTType(k, mat):
    if k == 2:
        arr1 = np.array(Odd(mat))
        arr2 = arr1.reshape(-1)
        list1 = arr2.tolist()
        return list1
    elif k == 0:
        arr3 = np.array(Even(mat))
        arr4 = arr3.reshape(-1)
        list2 = arr4.tolist()
        return list2
    elif k == 4:
...
def DSL(n):
    dsl = []
    for i8 in range(0, LCN[n]):
        dsl.append(np.floor(LTS[n] * (i8 + 1) / LCN[n]) - np.floor(LTS[n] * i8 / LCN[n]))
    return dsl
