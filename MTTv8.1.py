...
"""print('각 과목 정보가 담긴 l x 5짜리 xlsx파일의 이름이 뭔가요?')"""
dfKindLecture = pd.read_excel('LectureValues.xlsx')
LV = dfKindLecture.values
LectureNames = LV[:, 0].tolist()
l = len(LectureNames)
KL = LV[:, 1:2].tolist()
LCN = LV[:, 3].tolist()
print(LCN)
...
"""print('요일의 정보가 담긴 5 x 2짜리 xlsx파일의 이름이 뭔가요?')"""
dfDV = pd.read_excel('DV.xlsx')
DV = dfDV.values
datenames = DV[:, 0].tolist()
DPN = DV[:, 1].tolist()
...
def CTType(k, SelP, mat):
    ...
    for j in range(0, len(Selp)):
        ...
        if k == 0 or k == 2 and not list({tuple(DATES)} - {tuple(i) for i in SDPD}):
...
for l1 in range(0, l):
    SLlist = SL.tolist()
    print(CGS(l1, SLlist))
    PS = np.array_split(CGS(l1, SLlist), LCN[l1])[0].tolist()
    print(PS)
    for lcn in range(LCN[l1]):
        for j in range(len(PS[lcn])):
            SC[PS[lcn][j]][l1+lcn] = 1
        KC.append(KL[l1])
...
for i in range(0, s):
    StS[i] = dfSC.iloc[i]['S']
dfSC.drop(['SLN','S'], axis=1, inplace=True)
...
# TType 설정 코드#
UDP = np.union1d(np.array(IPD), np.array(IPD) + 4)
DEPN = []
...
NEP1 = np.setdiff1d(np.arange(wpn), np.union1d(EP, VEP)).tolist()
NEP = NEP1.tolist()
NEPI = dates(NEP)
DMEP = []
for i in range(weekly):
...
def style(workbook, n, i, j, list5, arr1):  # workbook: wb, n:학생,nsn:새로운 시트,i:요일,j:교시, list2=list3[s1]
...
def sheetname(b, arr):
    return arr[np.where(arr[:,0] == b)][1] + '번째 학생'


def makingsheetdone(mat, wbname, arr1):
            ...
            while pN < RpL[-1] + 1:
                if cN == c:
                    ...
                    if sdca == 1:
                        ...
                        ScTD = dfScTd.values
                        FScT = ScTD.reshape(-1).tolist()
                        CpTs.append(CpT)
                        Avgtdw = 0
                        for s1 in range(s):
                            sctd = np.copy(ScTD[s1])
                            for t1 in range(wpn):
                                c1 = sctd[t1]
                                c2 = sctd[t1 + 1]
                                l1 = PIL(ICL, c1)
        ...
        if end == 1:
            if len(sortFScTs) == 0:
                ...
                for i in range(len(sortFScTs)):
                    workbookname = str(i + 1) + '번째 가능한 시간표'
                    ScT10 = np.array(sortFScTs[i]).reshape(s, wpn).tolist()
                    makingsheetdone(ScT10, workbookname, StS)
