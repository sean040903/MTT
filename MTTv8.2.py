...
cnow = 0
for l1 in range(l):
    SinL = np.nonzero(SL[:,l1])[0].reshape(-1)
...
def printmtt(n1, n2, list1):
    printone = 'Found:' + str(n1) + ' ' * (
            170 - math.floor(np.log10(max(1, n1))) - 1) + 'class:' + str(n2) + ' ' * (
                       172 - math.floor(
                   np.log10(max(1, n2))) - 1) + '각 분반별 배정된 시간:' + " ".join(map(str, list1))
