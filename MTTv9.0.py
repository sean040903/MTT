...
def CTType(k, SelP, arr):
    ...
    if k == 0 or k == 2:
        if np.all(np.isin(dates, SDPD)):
            ...
            if arr[i0] == 0:
                if SelP[0] + np.sum(DPN[dates[0]:dates[1]]) == SelP[2 - k // 2]:
                    arr1[i0] = 0
    return arr1
...
def GTest(arr1, arr2, arr3, arr4, arr5, arr6):
    Focus1 = np.flatnonzero(arr1[:, cN])
    if np.max(arr2[Focus1][:, arr3]) > 0:
        return arr6[np.min(Focus1)] + s * cN
    n2 = PIL(ICL, cN)
    if np.max(arr4[n2, arr3]) < arr5[n2]:
        if cN in LB:
...
def plusingarr(arr, xind, yind, arr1):
    for i in xind:
        for j in yind:
            arr[i, j] += arr1[i, j]
...
def Even(arr):
    ...
    for i in range(weekly):
        if i not in CAD or arr[SDPI[np.where(CAD == i)[0]]] == 1:
            eap = np.append(eap, np.arange(IPD[i], arr[i] + 2, 2))
    return np.sort(eap)
...
def Odd(arr):
    ...
    for i in range(weekly):
        ...
        if i in CAD:
            if n1 == 1:
                oap = np.append(oap, np.arange(IPD[i], arr[i] + 2, 2))
        else:
            oap = np.append(oap, np.arange(IPD[i], arr[i] + 2, 2))
...
def printmtt(n1, n2, n3, arr1):
    printone = 'Found:' + str(n1) + '    checked SC: ' + str(n2) +'    back: ' + str(back) + ' ' * (
            48 - math.floor(np.log10(max(1, n1))) - 1 - math.floor(np.log10(max(1, n2))) - 1) + 'class:' + str(n3) + ' ' * (
                       76 - math.floor(np.log10(max(1, n3))) - 1) + '각 분반별 배정된 시간:' + " ".join(map(str, arr1))
...
while dead == 0:
    ...
    if out == 1:
        ...
        old = 0
        RE = np.array([], int)
        GT = np.zeros((g, wpn), int)
        RGT = np.empty((0, g, wpn), int)
        SbD = np.zeros((s, weekly), int)
        RSbD = np.empty((0, s, weekly), int)
        RpN = np.array([], int)
        RpL = np.array([], int)
        ...
        while out == 0:
            ...
            while go == 0:
                if pN == RpL[-1]:
                    ...
                    gtest = GTest(GC, GT, SelP, LP, MLP, GS)
                    if gtest == -1:
                        go = 1
                        RpN = np.append(RpN, pN)
                        printmtt(Found, len(RSC), cN, RpN)
                        RLP = np.append(RLP, [LP], axis=0)
                        plusing(LP, lN, SelP, 1)
                        RGT = np.append(RGT, [GT], axis=0)
                        plusing(GT, np.flatnonzero(GC[:, cN]), SelP, 1)
